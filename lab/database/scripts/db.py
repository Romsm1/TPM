...
class Data:
    ...
    def __new__(cls, filename):
        if cls._instance is None:
            ...
            cls.connect()
        return cls._instance # функция new не возвращала объект
    ...
    def add_order(self, **kwargs):
        try:
            sqlite_insert_query = """
                ...
                VALUES (?, ?, ?, ?, ?, ?); 
            """ # добавлен шестой параметр в VALUES
    ...
    def get_executors(self):
        try:
            request = "SELECT id_employee, surname FROM Employees" # орфографическая ошибка (surnam)
            self.data = self.cur.execute(request).fetchall()
        except sqlite3.Error as e:
            self.data = e
    ...
    def get_statuses(self):
        try:
            request = "SELECT id_status, status FROM Statuses" # орфографическая ошибка (Statuse)
            self.data = self.cur.execute(request).fetchall()
        except sqlite3.Error as e:
            self.data = e