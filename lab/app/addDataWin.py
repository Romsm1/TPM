...
class AddDataWin(QWidget):
    ...
    def add_order(self):
        ...
        # Валидация
        if not description or not acceptance_date or not customer:
            QMessageBox.warning(self, "Ошибка", "Описание, дата и заказчик должны быть заполнены")
            return

        if self.data:
            id_order = self.data[0]
            answer = self.db.update_order(
                type_of_work=type_of_work, description=description,
                acceptance_date=acceptance_date, customer=customer,
                executor=executor, status=status, id_order=id_order
            )
        else:
            answer = self.db.add_order(
                type_of_work=type_of_work, description=description,
                acceptance_date=acceptance_date, customer=customer,
