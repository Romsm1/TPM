...
def step_open_main_window(context):  # Шаг: открытие главного окна приложения
...
def step_clear_database(context):  # Шаг: очистка базы данных от всех записей
    students = context.db.get_all_students()
    for student in students:
        context.db.delete_student(student['id'])

@given('в базе данных есть студенты:')
def step_add_students_to_db(context):  # Шаг: добавление студентов из таблицы
...
def step_add_specific_student(context, name, age, grade):  # Шаг: добавление конкретного студента
    context.db.add_student(name, age, grade)

@given('в базе данных есть студент с именем "{name}"')
def step_add_student_by_name(context, name):  # Шаг: добавление студента только с именем
    context.db.add_student(name, 20, "хорошо")

@given('в базе данных есть "{count:d}" студента')
def step_add_number_of_students(context, count):  # Шаг: добавление указанного количества студентов
    for i in range(count):
        context.db.add_student(f"Студент {i+1}", 18 + i, "удовлетворительно")

@when('пользователь нажимает кнопку "{button_text}"')
def step_click_button(context, button_text):  # Шаг: нажатие кнопки с указанным текстом
...
def step_open_view_window(context):  # Шаг: открытие окна просмотра студентов
...
def step_enter_name(context, name):  # Шаг: ввод имени в поле ввода
    current_window = context.windows[-1]
...
def step_enter_age(context, age):  # Шаг: ввод возраста в поле ввода
    current_window = context.windows[-1]
...
def step_enter_grade(context, grade):  # Шаг: ввод оценки в поле ввода
    current_window = context.windows[-1]
...
def step_select_student(context, student_name):  # Шаг: выбор студента в таблице
    current_window = context.windows[-1]
...
def step_click_button_without_selection(context, button_text):  # Шаг: нажатие кнопки без выбора студента
    current_window = context.windows[-1]
    if hasattr(current_window, 'table'):
...
def step_enter_empty_name(context):  # Шаг: ввод пустого имени
    step_enter_name(context, "")

@when('пользователь вводит оценку ""')
def step_enter_empty_grade(context):  # Шаг: ввод пустой оценки
    step_enter_grade(context, "")

@when('пользователь изменяет имя на "{name}"')
def step_change_name(context, name):  # Шаг: изменение имени студента
    step_enter_name(context, name)

@when('пользователь изменяет возраст на "{age:d}"')
def step_change_age(context, age):  # Шаг: изменение возраста студента
    step_enter_age(context, str(age))

@when('пользователь изменяет оценку на "{grade}"')
def step_change_grade(context, grade):  # Шаг: изменение оценки студента
    step_enter_grade(context, grade)

@when('пользователь подтверждает удаление')
def step_confirm_delete(context):  # Шаг: подтверждение удаления студента
    pass

@when('добавляется новый студент через базу данных')
def step_add_student_via_db(context):  # Шаг: прямое добавление студента в БД
    context.db.add_student("Новый студент", 22, "хорошо")

@then('должно открыться окно добавления студента')
def step_verify_add_window_opened(context):  # Проверка: окно добавления должно открыться
    current_window = context.windows[-1]
...
def step_verify_view_window_opened(context):  # Проверка: окно просмотра должно открыться
    current_window = context.windows[-1]
...
def step_verify_success_dialog(context, message):  # Проверка: успешный диалог должен отобразиться
    students = context.db.get_all_students()
    assert len(students) > 0, "Студент не был добавлен в базу данных"

@then('должен отобразиться диалог ошибки с сообщением "{message}"')
def step_verify_error_dialog(context, message):  # Проверка: диалог ошибки должен отобразиться
    students = context.db.get_all_students()
...
def step_verify_student_saved(context, name, age, grade):  # Проверка: студент должен быть сохранен в БД
    students = context.db.get_all_students()
    found = False
    for student in students:
...
def step_verify_student_not_saved(context, name):  # Проверка: студент не должен быть сохранен в БД
    students = context.db.get_all_students()
    for student in students:
...
def step_verify_empty_student_not_saved(context):  # Проверка: студент с пустым именем не должен быть сохранен
    students = context.db.get_all_students()
    assert len(students) == 0, "В базе данных есть студенты, хотя не должно быть"

@then('в таблице должно отображаться "{count:d}" студента')
def step_verify_table_row_count(context, count):  # Проверка: количество строк в таблице должно соответствовать
    current_window = context.windows[-1]
    if hasattr(current_window, 'table'):
...
def step_verify_first_row_content(context, content):  # Проверка: содержимого первой строки таблицы
    step_verify_row_content(context, 1, content)

@then('вторая строка должна содержать "{content}"')
def step_verify_second_row_content(context, content):  # Проверка: содержимого второй строки таблицы
    step_verify_row_content(context, 2, content)

@then('третья строка должна содержать "{content}"')
def step_verify_third_row_content(context, content):  # Проверка: содержимого третьей строки таблицы
    step_verify_row_content(context, 3, content)

@then('четвертая строка должна содержать "{content}"')
def step_verify_fourth_row_content(context, content):  # Проверка: содержимого четвертой строки таблицы
    step_verify_row_content(context, 4, content)

def step_verify_row_content(context, row_number, content):  # Общая функция проверки содержимого строки таблицы
    current_window = context.windows[-1]
    if hasattr(current_window, 'table'):
        table = current_window.table
...
def step_verify_student_updated(context, name, age, grade):  # Проверка: студент должен быть обновлен в БД
    step_verify_student_saved(context, name, age, grade)

@then('студент "{name}" не должен быть в базе данных')
def step_verify_student_not_in_database(context, name):  # Проверка: студента не должно быть в БД после удаления (НАЖАТЬ ДА ВО ВРЕМЯ УДАЛЕНИЯ)
    time.sleep(1)
    students = context.db.get_all_students()
    print(f"=== ОТЛАДКА: В базе сейчас {len(students)} студентов ===")
    for s in students:
        print(f"  - {s['name']} (ID: {s['id']}, возраст: {s['age']}, оценка: {s['grade']})")
    print("========================================")
    for student in students:
        if student['name'] == name:
            assert False, f"Студент {name} все еще есть в базе данных (ID: {student['id']})"
    print(f"✓ Студент '{name}' успешно удален из базы данных")

@then('должен отобразиться диалог с предупреждением "{message}"')
def step_verify_warning_dialog(context, message):  # Проверка: диалог с предупреждением должен отобразиться
    pass

@then('в таблице должно отображаться "{expected_count:d}" студентов')
def step_verify_student_count_in_table(context, expected_count):  # Проверка: количество студентов в таблице должно соответствовать
    current_window = context.windows[-1]
    if hasattr(current_window, 'table'):
