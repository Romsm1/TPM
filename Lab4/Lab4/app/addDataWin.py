...
class AddDataWindow(QWidget):
    ...
    def save_data(self):
        ...
        if not name:
            QMessageBox.warning(self, "Ошибка!", "Требуется имя")
            return
        ...
        try:
            ...
            QMessageBox.warning(self, "Ошибка!", "Возраст должен быть положительным целым числом")
            return

        if not grade:
            QMessageBox.warning(self, "Ошибка!", "Требуется оценка")
            return
        ...
        try:
            if self.student_id:
                ...
                if success:
                    QMessageBox.information(self, "Выполнено!", "Студент успешно обновлен")
                else:
                    QMessageBox.warning(self, "Ошибка!", "Не удалось обновить данные студента")
            else:
                student_id = self.db.add_student(name, age, grade)
                QMessageBox.information(self, "Выполнено!", f"Студент успешно добавлен (ID: {student_id})")

            self.close()
        except Exception as e:
            QMessageBox.critical(self, "Ошибка!", f"Не удалось сохранить студента: {str(e)}")